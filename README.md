<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=32&pause=1000&color=1F6FEB&center=true&vCenter=true&width=600&height=60&lines=Hi%2C+I'm+Chavs+👋;Full+Stack+Developer;System+Builder;Based+in+Almaty%2C+KZ" alt="Typing SVG" />

  <p>
    <a href="https://github.com/Chavs23">
      <img src="https://komarev.com/ghpvc/?username=Chavs23&label=Profile%20Views&color=1f6feb&style=flat-square" alt="Profile Views" />
    </a>
  </p>
</div>

---

## 👨‍💻 The Engineer

I am focused on building scalable, production-grade systems from the ground up. I bridge the gap between high-performance backends and modern, fast frontends, always keeping deployment and infrastructure in mind.

**My Philosophy:**
- 🏗️ **Clean Architecture:** Designing for tomorrow, not just for today.
- ⚡ **Performance:** Building robust APIs and microservices.
- 🔄 **Automation:** CI/CD, containerization, and reducing manual toil.
- 🧠 **Long-term Thinking:** Choosing stability over hype.

🌱 **Currently exploring & building:** High-performance backends (Go), modern web platforms (Astro/React), and AI-integrated bots.

---

## 🛠️ Tech Arsenal

<div align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=go,nodejs,ts,js,react,astro,tailwind,docker,linux,kubernetes,postgres,redis,nginx,git,github&perline=8" alt="Tech Stack" />
  </a>
</div>

---

## 🚀 Recent Work & Projects

* 🚗 **Car Rental Aggregator (Dubai):** Architected scalable backend infrastructure.
* 🎨 **Artelle Studio:** Core development for technical portfolio & client solutions.
* 💸 **Pxp Platform:** Designed system architecture for a grant-funded platform.
* 🎣 **ai-phish-tank:** Configured containerized Docker environment for AI analysis.
* 🤖 **Custom AI Bots:** Developed tailored AI-integrated bots for freelance clients.

---

## 📊 GitHub Analytics

<div align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=Chavs23&show_icons=true&theme=github_dark&hide_border=true&bg_color=0D1117&border_radius=0" alt="GitHub Stats" />
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Chavs23&layout=compact&theme=github_dark&hide_border=true&bg_color=0D1117&border_radius=0" alt="Top Languages" />
</div>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Chavs23&theme=github-dark&hide_border=true&bg_color=0D1117&border_radius=0" alt="Activity Graph" />
</div>

---

## 🧭 Core Principles

<div align="center">

`Systems > Scripts` • `Clarity > Cleverness` • `Stability > Hype` • `Automation > Manual`

</div>

---

## 📫 Let's Connect

<div align="center">
  <a href="https://t.me/lxvsshall">
    <img src="https://img.shields.io/badge/Telegram-2CA5E0?style=flat-square&logo=telegram&logoColor=white" alt="Telegram" />
  </a>
  <a href="https://threads.net/@morda.dev">
    <img src="https://img.shields.io/badge/Threads-000000?style=flat-square&logo=threads&logoColor=white" alt="Threads" />
  </a>
  <a href="mailto:devmorda@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" alt="Email" />
  </a>
</div>

<br/>

<p align="center">
  <i>Engineering is about responsibility, not just code.</i>
</p>
